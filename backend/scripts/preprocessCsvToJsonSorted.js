...
const outputJsonPath = path.join(
  __dirname,
  "../data/BTW25/gemeindeIndexSorted.json"
);
...
fs.createReadStream(csvFilePath)
  ...
  .on("end", () => {
    console.log("CSV-Datei erfolgreich verarbeitet. Sortiere und speichere JSON...");

    // Alphabetische Sortierung der Schlüssel
    const sortedGemeindeIndex = Object.keys(gemeindeIndex)
      .sort()
      .reduce((acc, key) => {
        acc[key] = gemeindeIndex[key];
        return acc;
      }, {});

    // JSON-Datei speichern
    fs.writeFileSync(outputJsonPath, JSON.stringify({ gemeindeIndex: sortedGemeindeIndex }, null, 2));
